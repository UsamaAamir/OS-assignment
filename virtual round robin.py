...
IOwaiting=eval(input("what will be the waiting time for I/O processes"))
name=[]
arrivaltime=[]
bursttime=[]
starttime=[]
        ...
        if returntime:
            for j in range(count1):
                if finishtime[i-1]>=returntime[j]:
                    temporary = bursttime - IOtimearr[returnindex[j]]
                    if temporary==0:
                        ...
                        if IOtimearr[returnindex[j]] > quantum and bursttime[returnindex[j]] > quantum:
                            starttime.append(finishtime[returnindex[j]])
                            bursttime[returnindex[j]] = bursttime[returnindex[j]] - quantum
                            name.append(name[returnindex[j]])
                            bursttime.append(bursttime[returnindex[j]])
                            IOtimearr.append(IOtime)
                            finishtime.append(finishtime[returnindex[j]] + quantum)
                            process = process + 1
                            IOtimearr[returnindex[j]] = IOtimearr[returnindex[j]] - quantum

                        elif IOtimearr[returnindex[j]] > bursttime[returnindex[j]] and bursttime[returnindex[j]] < quantum:
                            starttime.append(finishtime[returnindex[j]])
                            finishtime.append(finishtime[returnindex[j]] + bursttime[returnindex[j]])
                            print("The process ", name[returnindex[j]], " started at ", starttime[returnindex[j]], " and ended at ",
                                  finishtime[returnindex[j]])

                        elif IOtimearr[returnindex[j]] < bursttime[returnindex[j]] and IOtimearr[returnindex[j]] < quantum:
                            starttime.append(finishtime[returnindex[j]])
                            finishtime.append(finishtime[returnindex[j]] + IOtimearr[returnindex[j]])
                            returntime.append(finishtime[returnindex[j]] + IOwaiting)
            ...
            for j in range(count1):
                ...
                if temporary == 0:
                    ...
                    if IOtimearr[returnindex[j]] > quantum and bursttime[returnindex[j]] > quantum:
                        ...
                        starttime.append(finishtime[returnindex[j]])
                        finishtime.append(finishtime[returnindex[j]] + bursttime[returnindex[j]])
                        print("The process ", name[returnindex[j]], " started at ", starttime[returnindex[j]],
                              " and ended at ",
                              finishtime[returnindex[j]])
